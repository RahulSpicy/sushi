...
class TestItemDetailView:
    ...
    def test_put(self, make_item):
        ...
        client = APIClient()
        client.force_authenticate(user_admin)
        response = client.put("/boards/items/1/", {"title": "Put", "list": 1})
        assert response.status_code == 200 and response.data["title"] == "Put"
        response = client.put("/boards/items/2/", {"title": "Put", "list": 3})
        assert (
            response.status_code == 200
            and response.data["title"] == "Put"
            and response.data["list"] == 3
        )
        client.force_authenticate(user_member)
        response = client.put("/boards/items/1/", {"title": "Put"})
        assert response.status_code == 403
        response = client.put("/boards/items/2/", {"title": "Put back", "list": 2})
        assert (
            response.status_code == 200
            and response.data["title"] == "Put back"
            and response.data["list"] == 2
        )
        client.force_authenticate(user2)
        response = client.put("/boards/items/1/", {"title": "Put"})
        assert response.status_code == 403
